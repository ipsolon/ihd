...
#define RAW_DATA_FILENAME   "/home/jmeyers/titan/samples/raw_packets"
/* --- end future command line options */

#if 0
uint8_t raw_packet[] = { // Single Raw packet captured from x310
        0x03, 0x00, 0xc0, 0x05, 0x00, 0x00,0xe0, 0x00,
        0xaa,0xea, 0x4d, 0x18, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
static std::queue<chameleon_fw_comms_t> buffer;
...
typedef struct map_file {
    ...
    uint8_t *mem;
    std::string filename;
    explicit map_file(const char *name, size_t pack_size) : fd(-1),
                                                      size(0),
                                                      num_packets(0),
...
void unmap_file(map_file_t &map)
{
    if (map.mem != nullptr) {
...
int map_file(map_file_t &map)
{
    int err = 0;
    struct stat s{};

    err = stat(map.filename.c_str(), &s);
    if (err) {
...
[[noreturn]]
void stream_thread_func(int sock_fd)
{
    int err = 0;
    ...
    std::string n(RAW_DATA_FILENAME);
    map_file_t sample_map(n.c_str(), RAW_PACKET_SIZE);
    err = map_file(sample_map);

    uint8_t *ptr = sample_map.mem;
    uint8_t *last_packet = sample_map.mem + ((sample_map.num_packets - 1) * sample_map.packet_size);
    while(true) {
        if(stream_run && !err) {
            ssize_t n = ::sendto(sock_fd, ptr, sample_map.packet_size, 0,
            ...
            if (n < 0) {
                ...
                if (ptr == last_packet) {
                    ptr = sample_map.mem; // back to the start
                } else {
                    ptr += sample_map.packet_size;
                }
            }
        } else {
            usleep(500000);
...
int IHD_SAFE_MAIN(int argc, char *argv[])
{
    ...
    if (!err) {
        ...
        std::thread vita_thread(&stream_thread_func, vita_sock_fd);
