...
public:
     ...
     *
     * \param buff frame buffer containing packet data
     *
     ...
     */
    [[nodiscard]] virtual uhd::transport::adapter_id_t get_send_adapter_id() const = 0;
     ...
     */
    [[nodiscard]] virtual bool supports_send_buff_out_of_order() const
    {
        return true;
    }
...
public:
    virtual ~recv_link_if() = default;
    using sptr = std::shared_ptr<recv_link_if>;

    /*!
     * Get the number of frame buffers that can be queued by this link.
     */
    [[nodiscard]] virtual size_t get_num_recv_frames() const = 0;

    /*!
     * Get the maximum capacity of a frame buffer.
     */
    [[nodiscard]] virtual size_t get_recv_frame_size() const = 0;
     ...
     *
     * \param buff frame buffer to release for reuse by the link
     */
     ...
     * Get the physical adapter ID used for this link
     */
    [[nodiscard]] virtual uhd::transport::adapter_id_t get_recv_adapter_id() const = 0;

    /*!
     * Returns whether this link type supports releasing the frame buffers
     * in an order different from that in which they were acquired.
     */
    [[nodiscard]] virtual bool supports_recv_buff_out_of_order() const
    {
        return true;
    }
