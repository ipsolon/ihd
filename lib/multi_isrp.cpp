...
public:
    explicit multi_isrp_impl(uhd::device::sptr dev) : multi_isrp(), _dev(std::move(dev)) {}
    ~multi_isrp_impl() override = default;

   uhd::device::sptr get_device() override { THROW_NOT_IMPLEMENTED_ERROR(); }

   [[nodiscard]] uhd::property_tree::sptr get_tree() const override { THROW_NOT_IMPLEMENTED_ERROR(); }
