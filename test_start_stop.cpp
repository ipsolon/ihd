...
int IHD_SAFE_MAIN(int argc, char *argv[]) {
    ...
    //thread_obj1 = new std::thread(&RxStream::start_stream, rxStream1);  // NOLINT
    //thread_vector.push_back(thread_obj1);
    //stream_vector.push_back(rxStream1);
    ...
    //thread_obj1 = new std::thread(&RxStream::start_stream, rxStream2);  // NOLINT
    //thread_vector.push_back(thread_obj2);
    //stream_vector.push_back(rxStream2);
    for (int i=0; i<5; i++) {
        rxStream1->start_stream();
        std::cout << "AFTER start_stream1 loop " << i << std::endl;
        rxStream2->start_stream();
        std::cout << "AFTER start_stream2" << std::endl;
        usleep(1000000);
        rxStream1->stop_stream();
        std::cout << "AFTER stop_stream1" << std::endl;
        rxStream2->stop_stream();
        std::cout << "AFTER stop_stream2" << std::endl;
      }
    free(rxStream1);
    free(rxStream2);
#if 0
